...
{ lab: 1-d bidomain, FN AP 
    Modified for lab, 1D active fiber in 2D region with point electrodes
}
...
definitions
  dphi_semi = table("dphi_semi.tbl")
  dV = table("dV.tbl")

  len = 0.6	{ cm, half-length of the fiber }
  c = 0.1       { cm, location of the electrodes }
  d = 0.6       { cm, width of the semi-infinite medium }
  ...
  Ji = -gp*dx(Vm)
  VAB_semi = INTEGRAL(Ji*dphi_semi)*h^2
  VAB_finite = INTEGRAL(Ji*dV)*h^2
...
elevation(dphi_semi) as "Imported semi-infinite lead field"
elevation(dV) as "Imported finite lead field"  

Histories
  history(vm) at (0,0) as 'Transm potential vs time'
  history(VAB_semi, VAB_finite) as 'Semi-infinite and finite VAB'
end
