TITLE 
    'Head Model - 3 concentric spheres, with Electrodes corrections'
...
DEFINITIONS
	...
	R_br = 8
    r = sqrt(x^2+y^2)	
	CD = -sigma*grad(V)	{ current density }
	{ electrode positions }
	{ Fc1 }
	x_Fc1 = 3.511
	y_Fc1 = 3.502
...
{ 'Ground electrode' }
{
	...
	Limited Region 3 { the skull region }
...
{
	contour(V) on x=0 on 'L3' as 'Potential on YZ plane through diameter in brain'
	contour(V) on y=0 on 'L3' as 'Potential on XZ plane through diameter in brain'
	contour(V) on z=0 on 'L3' as 'Potential on XY plane through diameter in brain'
}
	contour(V) on 'ST1' as 'Potential along top of the head' REPORT(SURF_INTEGRAL(normal(CD), 'ST1', 'ELECTRODE1')) AS 'Electrode 1 current' REPORT(SURF_INTEGRAL(normal(CD), 'ST1', 'ELECTRODE2')) AS 'Electrode 2 current'
