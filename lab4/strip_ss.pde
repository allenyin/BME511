...
{ lab: bidomain, iso+hom, time-dependent, FN model }
...
definitions
  ...
  phi0 = -gp/(4*pi*sig0)*integral(dot(grad(Vm), grad(rinv)))
...
boundaries
  region 1 'model'	{ over entire model }
    ...
    natural(Vm) = 0	
    line to (len,0)
    natural(Vm) = -js/ge*upulse(t, t-dur) { right electrode, time-dependent stimulus }
    line to (len,w)
    natural(Vm) = 0
    line to (-len,w)
    natural(Vm) = js/ge*upulse(t, t-dur) { left electrode, time-dependent stimulus }
    line to (-len,0)
    natural(Vm) = 0	
    line to close
...
plots
  for T=0 by tmax/20 to tmax
      contour(Vm) as 'Transmembrane potential'
